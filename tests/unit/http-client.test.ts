...
import { createMockFetch, emptyResponse, expectBearerToken, jsonResponse } from "../support/mock-fetch";

describe("http transport", () => {
  test("adds authorization only for private endpoints", async () => {
    ...
    const client = new TipplyClient({ accessToken: "token-123", fetch });

    await client.identity.getCurrentUser();
    await client.public.getWidgetMessage("user-123");

    expectBearerToken(requests[0]!.headers, "token-123");
    expect(requests[1]!.headers.get("authorization")).toBeNull();
  });

  test("prefers explicit access token over async token provider", async () => {
    let providerCalls = 0;
    ...
    expectBearerToken(requests[0]!.headers, "static-token");
  });

  test("uses async token provider when no static token is configured", async () => {
    const { fetch, requests } = createMockFetch(() => jsonResponse(currentUserFixture));
    const client = new TipplyClient({
    ...
    expectBearerToken(requests[0]!.headers, "async-token");
  });
  ...
  test("maps oauth failures to TipplyAuthError", async () => {
    ...
    const client = new TipplyClient({ accessToken: "bad-token", fetch });
